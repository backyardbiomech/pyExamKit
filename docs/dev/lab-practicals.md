...
Step 1 is built: `practical.py` (parser, validation, `to_key_data`, `sync_answers`), `keyformat.load_key_file` and `save_key_file` accepting a practical `.md`, `tools/practical_from_keys.py`, and `tests/test_practical.py`.

Step 2 is built except for the physical check: `sheet_layout` practical geometry and `read_form`, `answer_sheet.build_practical_sheet`, `practical_build.py` writing each form's sheet and a combined dealing-order PDF, and `tests/test_practical_sheet.py`. No sheet has been printed or scanned on real hardware. In its place, `tools/make_practical_test_stack.py` fills sheets in software (handwriting fonts, pencil IDs, a mix of right, misspelled, partial, wrong, and blank answers, and a few edge cases) and degrades them like a copier scan: skew up to 1.2 degrees, offset, scale, blur, noise, 200 dpi JPEG in a PDF. On the 12-student fixture stack (`tests/fixtures/practical/practical_test.md`), all 24 pages align through the scanner's own split and registration, and every layout, form, and ID reads correctly, including a very light pencil ID (fill level 0.22). `tests/test_practical_stack.py` repeats this on a two-student stack. Software scans are cleaner than a real copier (no paper curl, toner streaks, or feed skew past a few degrees), so a real scan is still worth doing before a real exam. The scanner does not yet route practical pages anywhere (step 4).
