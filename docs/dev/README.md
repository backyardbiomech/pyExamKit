...
- [outputs-cleanup.md](outputs-cleanup.md) — why a scan writes one gradebook and one Canvas file however many versions it held, how keys record each question's place in the bank so shuffled versions can be analyzed question by question, and what the item analysis computes.
