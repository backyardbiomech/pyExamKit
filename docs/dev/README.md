...
- [pdf-output-plan.md](pdf-output-plan.md) — why the exam is written as a PDF with PyMuPDF's Story rather than fpdf2 or HTML, how a question is kept whole on one page when MuPDF itself will not do it, and the decisions on math, footers, and font sizes.
