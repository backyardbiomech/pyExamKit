...
**Use same questions across all versions** changes what "version" means. Left off, each version draws its own fresh random sample from the pools, so version B may ask about things version A never mentions. Turned on, all versions ask the identical set of questions and differ only in order. Same-questions versions are the ones to use when the versions have to be comparable to each other. They also share one answer sheet when the exam has short-answer, ordering, matching, or dropdown questions: version A is shuffled freely, and every other version keeps each of those questions at version A's position and shuffles only the questions between them, so the writing boxes and the gaps around grouped questions are on the same rows for everyone. The items within an ordering or matching question are still shuffled per version.
...
It also writes `Title_answer_sheet.pdf`, the [answer sheet](answer-sheets.md) for this exam: sized to its number of answer rows, headed with the course and title, with a numbered writing box for each short-answer question, and with the rows of each ordering, matching, and dropdown question set off together by gaps. The key records where each box is and where the rows are, so the scanner crops the handwriting without being told; a sheet with grouped rows must be scanned with its key file (see [Answer sheets](answer-sheets.md#rows-grouped-by-question)). When versions draw different questions, their short-answer and grouped questions can land on different rows, and each version then gets its own `Title_vA_answer_sheet.pdf`, marked "Form A" in small print at the bottom. Each must be handed out with its own exam, so turn on **Use same questions across all versions** when you can. The log says which, and warns if the written questions do not fit (the boxes need an exam of 90 or fewer answer rows).
