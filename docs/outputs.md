...
**`gradebook.xlsx`** is the working copy of the grades. Each question gets an answer column and a points column, the key is on a highlighted row at the top with each question's points beside it, and **each student's total is a live `SUM` formula**, so changing a points cell after a regrade discussion updates the total immediately. The key row shows the acceptable answers for written questions, pipe-separated. Students are listed in the order they were scanned. With a key made by Build Exam, a **Question** row under the headers gives each question's text, with the part a row grades (a dropdown, an ordering position, a matching item) in brackets after it.

Three rows under the students summarize each question's points column: the **Mean** and **Median** points earned, and Canvas's **Discrimination index**. The index splits the class by total score and subtracts the mean fraction of the question's points earned by the bottom 27% from that earned by the top 27%, so it runs from −1 to +1: +1 when every top student earned the points and no bottom student did, 0 when both groups did equally well, and negative when the bottom group did better. Canvas treats 0.25 and above as good. Students tied with the one at a cutoff all join that group, so with a small class and many tied totals the groups can hold more than 27% of it. All three rows are formulas, like the totals, so they follow any points you correct. A student whose form did not include a question (a lab practical) has a blank rather than a zero there and is left out of that question's numbers. The **By question** tab (below) has the same rows, across versions.

The first tab, **Exam stats**, summarizes the totals: the number of students, the points possible, the mean, median, standard deviation, highest and lowest score, with the mean, median, highest and lowest also as a percent of the points possible. With several versions it has a column for the whole class and one per version; when versions are worth different totals, the class column leaves out the percents, since no one total applies.

**Reliability** is Cronbach's alpha: how consistently the questions rank students, from each question's variance against the variance of the totals. On questions scored all or nothing it is the same number as KR-20. It runs up to 1, rises with the number of questions, and can fall below 0 when questions disagree. The class figure needs every version's questions matched, so it comes from the By question tab and is blank for versions whose keys cannot be matched. A lab practical leaves blanks for questions not on a student's form, and alpha is only approximate when data are missing that way.

Below that, the **grade distribution** counts students in each letter grade by percent of the points possible, each student against their own version's total: A 93–100%, A− 90–92%, then B, C, and D each split into − (80–82%), plain (83–86%), and + (87–89%) the same way, and F split into close (50–59%) and low (below 50%). Each band includes its lower bound and stops short of the next, so 82.9% is a B−. A bar chart shows the same counts. Every number on the tab is a formula over the gradebook tabs, so it follows any points you correct.

The workbook's last tab, **Item analysis**, has one row per question: its text (with a key made by Build Exam), its key, its points, how many students answered it, the mean score (the fraction of its points students earned on average, so 0.9 is an easy question), its discrimination, and how many students marked each choice. The key's letters are bold among the choice counts, so a distractor that drew more students than the key stands out.

Discrimination here is not the index on the gradebook tabs: it is the correlation between a student's score on the question and their score on the rest of the exam. A good question is answered correctly more often by students who did well overall, and has a positive discrimination. A question near zero does not separate strong students from weak ones, and a negative one was answered correctly more often by weaker students; that usually means a wrong key, an ambiguous stem, or a distractor that is defensibly right. Questions below 0.2, a common rule of thumb, are shaded and marked in the **Look at** column. With a small class these numbers move a lot from one student to the next, so read them as a pointer to questions worth rereading, not a verdict.
